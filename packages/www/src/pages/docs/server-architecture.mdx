...
<br />
The default translation is performed according the [httpc call convention](httpc-call-convention), which defines simple rules to identify which server function to call and extract the eventual arguments.

<br />
Any incoming request must match a function, otherwise the _HTTP 404_ is returned.

<Aside type="info">
httpc-aware clients make requests already compliant to the [call convention](httpc-call-convention) so no configuration is needed server side.<br/>
***
...
Read a deep dive with detailed explanation at [Request context](server-request-context).
</Aside>
...
********
If you need a more opinionated approach, you should check [@httpc/kit](kit-introduction) which offers builtin support for a broad range of business concerns like authentication, authorization, validation, caching, ...
</Aside>
...
<br />
See [Extending](./server-extending) for details.
...
<br />
By default, the **@httpc/server** uses the builtin [`HttpCCallParser`](#httpccallparser), which follows the [httpc call convention](httpc-call-convention) to make calling a server function very easy.
<br />
...
It transforms an _http request_ into an `HttpCall` by following the [httpc call convention](httpc-call-convention). 
<br/>
Details and configuration options are available in [reference](package-httpc-server).

### Builtin parsers
In addition to json API, the [@httpc/server](package-httpc-server) package offers several parsers covering additional use cases. For example, `FormUrlEncodedParser` to handle form submissions, or `RawBodyParser` to handle binary data.
<br/>
You can check the [reference](package-httpc-server) for details.
...
<Aside type="info">
The [@httpc/kit](kit) provides many middlewares ready to use for different use cases like authentication, validation, catching ...
</Aside>
...
### Result
Anything a function handler returns will be the result. An handler can also throw an error to halt the execution. There are several [builtin errors](package-httpc-server#errors) that map to the relative http error like `BadRequestError`, `UnauthorizedError` and so on.
<br/>
