...
<Aside type="info">
The **httpc call convention** is implemented by the builtin `HttpCCallParser` which is included by default with any [httpc server](server-architecture) configuration.
*** 
An **httpc server** can handle any _http request_, even when non **httpc** compliant, with [custom parsers](server-extending#custom-parser).
</Aside>
...
- [Strict](#strict-mode)
  ...
  This is useful when receiving requests from _httpc-aware_ clients like `HttpCClient` or the custom client [generated](client-generation) from an API.
...
The httpc framework provides [helpers](kit-caching) to set http caching headers with flexible configurations.
</Aside>
