...
## Custom Parser
A parser is typed by `HttpCServerCallParser`. It transforms an _http request_ into an [`HttpCall`](server-architecture#parsing). A parser can skip the processing if it doesn't support the request. The server will try the next parser in the list.
...
<Aside type="tip">
By default the server includes the builtin [HttpCCallParser](package-httpc-server) that parses the request following the [httpc call convention](httpc-call-convention). The `HttpCCallParser` is added last, so any parser defined takes precedence.
</Aside>
...
<Aside type="tip">
[@httpc/server](package-httpc-server) offers some [builtin parsers](package-httpc-server#parsers) like the `FormUrlEncodedParser` to handle browser form submission, or the `RawBodyParser` to handle binary payloads.
***
Before implementing a new parser, please check if a [builtin parser](package-httpc-server#parsers) covers use case.
</Aside>
...
Additional examples are available in the [tutorials](tutorials).
